...
| `browser-support-regression` | Cross-platform browser support regression | One required Android device running Chrome, Edge, Brave, and Firefox; optional Windows coverage may use one VM running Chrome, Edge, Brave, Opera, and Firefox; 5 gambling and 5 non-gambling fixtures per browser. These are evaluation candidates, not a current support claim. Evidence is grouped as `<platform>/<browser>/<case>` under the client-runtime evidence root. | Android browser cells are gating; Windows browser cells are optional and non-gating. Runtime evidence must record aggregate `allow` for non-gambling and `intervention` for gambling. This is separate from anti-uninstall and latency evidence. |
