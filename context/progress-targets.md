...
| `browser-support-regression` | Cross-platform browser support regression | One required Android device running Chrome, Edge, Brave, Firefox, Samsung Internet, and Xiaomi Browser; optional Windows coverage may use one VM running Chrome, Edge, Brave, Opera, and Firefox; 5 gambling and 5 non-gambling fixtures per browser. Evidence is grouped as `<platform>/<browser>/<case>` under the client-runtime evidence root. | Android browser cells are gating; Windows browser cells are optional and non-gating. Runtime evidence must record aggregate `allow` for non-gambling and `intervention` for gambling. This is separate from anti-uninstall and latency evidence. |
...
| `anti-uninstall` | Android protection | The supported OEM/scenario matrix, including partner removal, two-admin emergency removal, and approved-removal cancellation recovery, is completed with valid lifecycle evidence. | Manual system UI evidence is required; partial AOSP coverage is not full matrix coverage. Android/OEM Settings limitations remain honest failures/limitations, not code defects. |
