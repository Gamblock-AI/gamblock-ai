...
| Android anti-uninstall | **partial, recorded**: 18 samples across 17 groups, with 16 passing assertions and 2 Android/OEM Settings limitations; the seven valid AOSP Pixel lifecycle records are a subset. Source now removes automatic Settings redirects and adds partner/emergency/cancellation hardening, but no new runtime evidence was executed. | Complete all required OEM-family and scenario cells in the expanded device matrix; retain Settings limitations as platform constraints, not assumed Flutter code defects. |
...
| Cross-platform browser support regression | **pending UPX expansion**: Chrome, Edge, Brave, Firefox, Samsung Internet, and Xiaomi Browser retain complete recorded 5+5 cells; UPX Browser now requires its own signed Research-release 5+5 batch on the same required Android device. | The existing six-browser outcomes remain aggregate-safe recorded evidence. Required Android coverage becomes complete again only when UPX records `allow` for all non-gambling samples and `intervention` for all gambling samples; Windows Chrome/Edge/Brave/Opera/Firefox remains optional and non-gating. |
...
Chrome/Edge/Brave/Firefox/Samsung Internet/Xiaomi Browser; UPX Browser remains
pending its signed Research-release 5+5 batch on the same required device.
