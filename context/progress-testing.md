...
| Cross-platform browser support regression | **passed (recorded)**: Chrome, Edge, Brave, Firefox, Samsung Internet, Xiaomi Browser, and UPX Browser each have complete recorded 5+5 cells on the required Android device. | All seven Android browser cells record `allow` for non-gambling samples and `intervention` for gambling samples; Windows Chrome/Edge/Brave/Opera/Firefox remains optional and non-gating. |
