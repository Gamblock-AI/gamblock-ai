...
| Android anti-uninstall | **partial, recorded**: 18 samples across 17 groups, with 16 passing assertions and 2 Android/OEM Settings limitations; the seven valid AOSP Pixel lifecycle records are a subset. Source now removes automatic Settings redirects, adds partner/emergency/cancellation hardening, and carries a target-bound 30-second session across resolved OEM removal surfaces; no new device-runtime evidence was executed. | Complete every scenario for AOSP/Pixel, Samsung, Xiaomi/Redmi, OPPO/realme/OnePlus, vivo/iQOO, and Transsion; retain Settings limitations as platform constraints, not assumed Flutter code defects. |
| Android latency (Redmi 12C, 30 samples) | **passed (recorded)** in run `redmi12c_release_probe` in the privacy-safe Phase 4 ledger for Research release, Chrome, `warm_foreground_online`; 30/30 visible and completed, p95 `input_to_visible_ms` 182.34 ms, no failures. | The current progress-demo checkpoint is **passed** for this homogeneous demonstrated group. The former final-readiness latency matrix has been replaced by the browser-support runtime contract. |
...
| Cross-platform browser support regression | **historical pass; current-source validation pending**: Chrome, Edge, Brave, Firefox, Samsung Internet, Xiaomi Browser, and UPX Browser have complete recorded 5+5 cells on the required Android device, but those runs predate dynamic browser discovery, generic OCR, committed-standalone surface gating, and the opaque-browser explanation fallback. | Preserve the seven recorded cells as historical evidence. Re-run the Android matrix against the changed Research source and record discovery, ignored Custom Tab/internal/transient surfaces, explanation-first-frame before Home, and native Activity fallback scenarios; Windows remains optional and non-gating. |
...
artifact contract; complete Android device matrix; and the approved structured
usability protocol if human-participant results are to be claimed. The required
Android browser regression has recorded 5+5 coverage for
Chrome/Edge/Brave/Firefox/Samsung Internet/Xiaomi Browser/UPX Browser on the
same required device. All seven Android cells passed; optional Windows
coverage remains separate and non-gating.
Optional Windows coverage may be added separately
with Chrome/Edge/Brave/Opera/Firefox. The former Android/Windows
Chrome latency matrix is no longer a separate gate. The isolated backend
integration gate is passed and recorded.
